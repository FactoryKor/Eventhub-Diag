...
def discover_checkpoint_store(cred, sub: str, rg: str,
                              fqdn: str) -> tuple[Optional[str], str]:
    """Best-effort auto-discovery of the BlobCheckpointStore container.

    Enumerates storage accounts in the namespace's resource group, then probes
    each container for the checkpoint blob prefix
    ``{fqdn}/{eventhub}/{consumer_group}/checkpoint/{partition_id}``.

    Returns ``(container_url, note)``. Requires *Storage Blob Data Reader* on the
    matching account (and Reader on the RG to enumerate accounts). Strictly
    read-only; stops at the first matching container and swallows per-account
    permission errors so one inaccessible account does not abort discovery.
    """
    stg = _lazy_import("azure.mgmt.storage")
    blobmod = _lazy_import("azure.storage.blob")
    if stg is None:
        return None, "azure-mgmt-storage 미설치로 체크포인트 자동 탐색 생략"
    if blobmod is None:
        return None, "azure-storage-blob 미설치로 체크포인트 자동 탐색 생략"
    prefix = f"{fqdn.lower()}/"
    try:
        smc = stg.StorageManagementClient(cred, sub)
        accounts = list(smc.storage_accounts.list_by_resource_group(rg))
    except Exception as e:  # noqa: BLE001
        return None, f"스토리지 계정 열거 실패: {str(e).splitlines()[0]}"
    for acct in accounts:
        try:
            blob_ep = (acct.primary_endpoints.blob if acct.primary_endpoints
                       else None) or f"https://{acct.name}.blob.core.windows.net/"
            svc = blobmod.BlobServiceClient(account_url=blob_ep, credential=cred)
            for cont in svc.list_containers():
                try:
                    cc = svc.get_container_client(cont.name)
                    for blob in cc.list_blobs(name_starts_with=prefix):
                        segs = blob.name.split("/")
                        if len(segs) >= 5 and segs[3] == "checkpoint":
                            url = blob_ep.rstrip("/") + "/" + cont.name
                            return url, f"자동 탐색: {acct.name}/{cont.name}"
                except Exception:  # noqa: BLE001
                    continue
        except Exception:  # noqa: BLE001
            continue
    return None, "리소스 그룹 내에서 체크포인트 컨테이너를 찾지 못함"
...
def evaluate(report: Report, control: dict[str, Any], metrics: dict[str, Any],
    ...
    if metric_err:
        ...
        for hub in targets:
            ...
            if inc and inc > 0 and out is not None:
                ...
                report.add("backlog", "info",
                           f"Egress/Ingress 균형 미평가 \u2014 {label}",
                           f"[{label}] IncomingMessages/OutgoingMessages 메트릭이 "
                           f"없어 egress/ingress 균형을 평가하지 못했습니다.",
                           {"event_hub": hub, "incoming": inc, "outgoing": out},
                           recommendation="Producer로 트래픽을 발생시키거나 메트릭 "
                                          "평면 접근 권한을 확인한 뒤 다시 실행하세요.")
            ...
            if active is None:
                report.add("connections", "info",
                           f"활성 연결 수 미평가 \u2014 {label}",
                           f"[{label}] ActiveConnections 메트릭이 없어 "
                           f"평가하지 못했습니다.",
                           {"event_hub": hub},
                           recommendation="메트릭 평면 접근 권한을 확인하거나 "
                                          "연결이 발생한 뒤 다시 실행하세요.")
    ...
    for hub, rt in (runtime_by_hub or {}).items():
        ...
        if len(retained) >= 2 and sum(retained) > 0:
            ...
            report.add("partition_skew", "info",
                       f"파티션 분포 편중 미평가 \u2014 {hub}",
                       f"[{hub}] 파티션에 보관 이벤트가 부족해 편중을 "
                       f"평가하지 못했습니다.",
                       {"event_hub": hub, "partitions": len(parts)},
                       recommendation="이벤트를 전송해 파티션에 데이터가 쌓인 뒤 "
                                      "다시 실행하세요.")
    ...
    # -- config audit (매 항목마다 결과를 남김: ok/경고/위험) ---------------- #
    if control and not control.get("_error"):
        tier = control.get("sku_tier")
        ai = control.get("auto_inflate_enabled")
        if tier == "Standard" and ai is False:
            ...
            report.add("config", "ok", "Auto-Inflate 활성화됨",
                       "Auto-Inflate가 켜져 있어 처리량 스파이크를 흡수합니다.",
                       {"sku_tier": tier, "auto_inflate": True})
        elif tier is not None:
            report.add("config", "ok", "Auto-Inflate 검사",
                       f"현재 티어({tier})에는 Auto-Inflate 경고가 해당하지 않습니다.",
                       {"sku_tier": tier, "auto_inflate": ai})

        pna = control.get("public_network_access")
        if pna == "Enabled":
        ...
        elif pna is not None:
            report.add("config", "ok", "공용 네트워크 액세스 차단",
        ...
        if tls is not None:
            ...
            if mtls and (int(mtls.group(1)), int(mtls.group(2))) < (1, 2):
                ...
                report.add("config", "ok", "최소 TLS 버전 양호",
                           f"minimumTlsVersion={tls} (1.2 이상).",
                           {"minimum_tls_version": tls})

        dla = control.get("disable_local_auth")
        if dla is False:
            ...
            report.add("config", "ok", "Entra 전용 인증",
                       "로컬(SAS) 인증이 비활성화되어 Entra 전용입니다.",
        ...
        report.add("config", "info", "구성 감사 미평가",
                   str(control["_error"]),
                   recommendation="ARM 제어 평면 접근 권한(Reader)과 --azure-auth를 "
                                  "확인한 뒤 다시 실행하세요.")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    cp.add_argument("--no-auto-checkpoint", action="store_true",
                    help="Disable best-effort auto-discovery of the checkpoint "
                         "store in the namespace's resource group (enabled by "
                         "default when --checkpoint-store is not given).")
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    lag_list: Any = []
    ckpt_store = args.checkpoint_store
    if (not ckpt_store and not args.no_auto_checkpoint
            and cred is not None and runtime_by_hub):
        ckpt_store, note = discover_checkpoint_store(cred, sub, rg, fqdn)
        if ckpt_store:
    ...
    if ckpt_store and runtime_by_hub:
        try:
            lag_list = collect_all_checkpoint_lag(
                ckpt_store, cred, fqdn, runtime_by_hub,
